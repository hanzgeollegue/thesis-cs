...
try:
    ...
    from .config import get_openai_config, get_llm_config, validate_config, USE_LLM_RANKER, PARSE_CONCURRENCY, BATCH_TIMEOUT_SEC
except ImportError:
...
class BatchProcessor:
    ...
    def process_batch(self, resumes: List[str], job_description: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 5: Cross-Encoder reranking with multiple features
            logger.info("Applying Cross-Encoder reranker with multiple features")
            try:
                # Pass raw job description for normalization within Cross-Encoder
                ce_results = self._apply_cross_encoder_reranking(parsed_resumes, job_description, 
                ...
                # Select top 50 for LLM processing
                try:
                    sorted_indices = sorted(range(len(ce_results)), key=lambda i: ce_results[i].final_score, reverse=True)
            ...
            if USE_LLM_RANKER:
                ...
                logger.info("LLM ranking disabled, using Cross-Encoder composite scoring")
                with time_phase("llm_rank", timing_bucket):
                    try:
                        final_ranking = self._generate_ce_composite_rankings(parsed_resumes)
                    except Exception as e:
    ...
    def _parse_pdfs_to_json(self, resume_paths: List[str]) -> List[ParsedResume]:
        ...
        logger.info(f"[PERF] Using {max_workers} workers for PDF parsing")
        try:
    ...
    def _apply_cross_encoder_reranking(self, resumes: List[ParsedResume], job_description: str,
        ...
        try:
            ...
            result = self.ce_reranker.rerank_candidates(job_description, resume_dicts, 
                                                      section_tfidf_scores, skill_tfidf_scores, sbert_scores)
